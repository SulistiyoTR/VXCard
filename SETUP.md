...
- Put it in `.env.local` as `ANTHROPIC_API_KEY`. It is only ever read in
  `src/app/api/generate/route.ts` / `src/lib/llm.ts` — server-side.

## 4. Vercel

1. Import the GitHub repo.
2. Add the same env vars (`NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`,
   `ANTHROPIC_API_KEY`, `NEXT_PUBLIC_SITE_URL` = the deployed URL).
3. Deploy. Open the URL on your iPhone → Safari → Share → **Add to Home Screen**.
...
| V2 (Fase 3): streak, stats, calendar, waiting indicator, regenerate sentences | ✅ in this PR |
| V3 (Fase 4): offline-first IndexedDB, web push notifications | ⏳ not started (architecture change — see BUILD-PLAN Sesi 20–23) |
...
- Added a `sessions` table (not in SPEC §5) — streak/calendar need it.
